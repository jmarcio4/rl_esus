-- View: rl_esus."MOTIVO_VISITA_DIARIA"

DROP VIEW if exists rl_esus."MOTIVO_VISITA_DIARIA";

CREATE OR REPLACE VIEW rl_esus."MOTIVO_VISITA_DIARIA" AS 
 SELECT tb_cds_prof.nu_cnes,
    ...
    count(
        CASE
            ...
            ELSE NULL::integer
        END) AS dia_31,
    count(tb_cds_visita_dom_motivo.no_cds_visita_dom_motivo) AS count
  ...
  GROUP BY tb_cds_prof.nu_cns, to_char(tb_cds_ficha_visita_domiciliar.dt_ficha, 'YYYY-MM'::text), ( SELECT tb_localidade.no_localidade AS no_municipio
           FROM tb_localidade
          WHERE tb_localidade.co_localidade = (( SELECT tb_adm_municipal.co_localidade
                   FROM tb_adm_municipal
                 LIMIT 1))), tb_cbo.co_cbo_2002, tb_cbo.no_cbo, tb_dim_unidade_saude.no_unidade_saude, pp.no_profissional, tb_cds_visita_dom_motivo.no_cds_visita_dom_motivo, tb_cds_prof.nu_cnes
  ORDER BY tb_cds_prof.nu_cnes, tb_cds_prof.nu_cns, to_char(tb_cds_ficha_visita_domiciliar.dt_ficha, 'YYYY-MM'::text);

ALTER TABLE rl_esus."MOTIVO_VISITA_DIARIA"
  OWNER TO postgres;
GRANT ALL ON TABLE rl_esus."MOTIVO_VISITA_DIARIA" TO postgres;
GRANT ALL ON TABLE rl_esus."MOTIVO_VISITA_DIARIA" TO pmaq;
GRANT ALL ON TABLE rl_esus."MOTIVO_VISITA_DIARIA" TO relatorio;
